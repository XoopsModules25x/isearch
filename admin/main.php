...
switch ($op) {
    ...
    case 'ConfirmBeforeToPrune':
        ...
        if ($count > 0) {
            ...
            xoops_confirm(['op' => 'pruneKeywords', 'keyword' => $keyword, 'prune_date' => $timestamp, 'ip' => $ip, 'ok' => 1], 'main.php', $msg);
        } else {
    ...
    case 'SearchExport':
        ...
        if (count($tbl) > 0) {
            ...
            foreach ($tbl as $onesearch) {
                fwrite($fp, $onesearch->getVar('isearchid')
                            . $delimiter
                            ...
                            . $delimiter
                            . $tmpisearch->uname($onesearch->getVar('uid'))
                            . $delimiter
                            . $onesearch->getVar('ip')
                            . "\r\n");
            }
    ...
    case 'MetagenBlackList':
        ...
        if (isset($_POST['go']) && _AM_ISEARCH_DELETE == $_POST['go']) {
            foreach ($_POST['blacklist'] as $black_id) {
                $blacklist->delete($black_id);
            }
            $blacklist->store();
        } else {
            if (isset($_POST['go']) && _AM_ISEARCH_BLACKLIST_ADD == $_POST['go']) {
 ...
 $start = \Xmf\Request::getInt('start1', 0, 'SESSION');
        ...
        */
        ...
        foreach ($elements as $oneelement) {
            ...
            $action_black     = '<a '
                                . isearch_JavascriptLinkConfirm(_AM_ISEARCH_AREYOUSURE)
                                . " href='main.php?op=addblacklist&id="
                                . $oneelement->getVar('isearchid')
                                . "' title='"
                                . _AM_ISEARCH_BLACKLIST
        ...
        echo "<tr><form method='post' action='main.php'><th align='center'>"
             . _AM_ISEARCH_FILTER_BY
